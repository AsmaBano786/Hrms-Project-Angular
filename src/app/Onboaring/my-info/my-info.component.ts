...
attachmentValid:boolean=false;
  ...
  ngOnInit(): void {
    ...
    this.onboardingdata=JSON.parse(sessionStorage.getItem('local_storage')|| "[]");  //recieve

    console.log("edit session storage",this.onboardingdata);
    
    for(let i in this.onboardingdata){
      this.emp_id= this.onboardingdata[i].emp_id;
    this.compid=this.onboardingdata[i].company_id
      console.log("emp_id..........",this.onboardingdata[i].emp_id);
...
getUpdateData()
{
  this.authService.GetCandidateDetailsByCid(this.data,this.compid).subscribe((result:any) => {
...
//      document.getElementById("summary")?.className=="tab-pane"

//     document.getElementById("info")?.className=="active tab-pane"
