...
import { TimesheetService } from '../../../Onboaring/time-sheet/services/timesheet.service';
...
@Component({
  selector: 'app-timesheet-approval-requests',
  templateUrl: './timesheet-approval-requests.component.html',
  styleUrls: ['./timesheet-approval-requests.component.css']
})
export class TimesheetApprovalRequestsComponent implements OnInit {
...
this.notesForm = this.formBuilder.group({
  ...
  employeeId: [this.findByid],
    ...
    FoundTSData() {
      this.timesheetService.getTimesheetByIdFound(this.findByid).subscribe(res => {
        console.log("Res", res);
        this.TSheetData = res;
  ...
  console.log("timesheetData",this.timesheetData1);
    ...
    getTotalHours() {
      this.timesheetService.getTotalHours(this.findByid).subscribe((res: any) => {
        console.log("Total Hours Res", res,this.findByid);
        this.totalWorkingHrs = res.TotalHours;
        console.log("Total Hours Res", this.totalWorkingHrs,this.findByid);
      }, (err) => {
    ...
    getdetail() {
      this.timesheetService.getTimesheet().subscribe((data: any): void => {
        ...
        console.log("getdetail......21", this.alldata.data);
    ...
    deleteTimesheet(j: any) {
      ...
      // if (confirm("Are you sure to delete?")) {
        this.timesheetService.deleteTimesheet1(j).subscribe(async (result) => {
          ...
          setTimeout(() => { this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => this.router.navigate(['/timesheet'])); }, 1000);
...
if(this.selectedattach=="Excel TimeSheet")
{
  ...
  GetAttachments() {
    ...
    this.timesheetService.getAttachbyEmp(this.findByid).subscribe((result: any) => {
  ...
  Getnotes() {
    ...
    this.timesheetService.getNotesbyEmp(this.findByid).subscribe(async (result) => {
...
//info
this.timesheetService.getTimesheetByIdFound(eid).subscribe(res => {
  console.log("Res", res);
  this.TSheetData = res;
  console.log("TSheetData..found..byID",this.TSheetData);
...
this.timesheetService.getAttachbyEmp(eid).subscribe((result: any) => {
...
//notes
this.timesheetService.getNotesbyEmp(eid).subscribe(async (result) => {
  console.log(this.findByid);
  // console.log(this.reqBody2);
...
//total hours
this.timesheetService.getTotalHours(eid).subscribe((res: any) => {
  console.log("Total Hours Res", res,eid);
  this.totalWorkingHrs = res.TotalHours;
  console.log("Total Hours Res", this.totalWorkingHrs,eid);
}, (err) => {
  console.log("Errors", err);
})
  ...
  goto(employeeId:any,TimeSheetId:any,action:any){
   ...
   console.log("check action..",Action_Val);
    ...
    this.timesheetService.UpdateTimesheetPendingReport(employeeId,TimeSheetId,Action_Val).subscribe((result)=>{
      console.log("api--->",result);
      alert("Thank You, Pending Action has been changed")
      // this.router.navigate(['/pending-report']);
      setTimeout(() => { this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => this.router.navigate(['/timesheet-approval-request'])); }, 2000);
